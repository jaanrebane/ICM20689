...
int ICM20689::setDlpfBandwidth(DlpfBandwidth bandwidth) {
	...
	case DLPF_BANDWIDTH_MAX: {
		...
		if (writeRegister(GYRO_CONFIG,
		      ((_buffer[0] & 0xFC) | GYRO_FCHOICE_B_8173HZ))
		    < 0) {  // setting gyro bandwidth to 250Hz
			return -2;
		}
		break;
	}
...
int ICM20689::enableDataReadyInterrupt() {
	...
	if (writeRegister(INT_PIN_CFG, INT_PULSE_50US) < 0) {  // setup interrupt, 50 us pulse
		// if (writeRegister(INT_PIN_CFG,INT_HOLD_ANY) < 0) { // setup interrupt,
		// hold, any read operation
		return -1;
	}
...
int ICM20689::readSensor() {
	...
	_acc[0] = (((double)(tX[0] * _accCounts[0] + tX[1] * _accCounts[1] + tX[2] * _accCounts[2]) * _accelScale) - _accB[0])
	        * _accS[0];
	_acc[1] = (((double)(tY[0] * _accCounts[0] + tY[1] * _accCounts[1] + tY[2] * _accCounts[2]) * _accelScale) - _accB[1])
	        * _accS[1];
	_acc[2] = (((double)(tZ[0] * _accCounts[0] + tZ[1] * _accCounts[1] + tZ[2] * _accCounts[2]) * _accelScale) - _accB[2])
	        * _accS[2];
	_t = ((((double)_tCounts) - _tempOffset) / _tempScale) + _tempOffset;
	_gyro[0] =
	  ((double)(tX[0] * _gyroCounts[0] + tX[1] * _gyroCounts[1] + tX[2] * _gyroCounts[2]) * _gyroScale) - _gyroB[0];
	_gyro[1] =
	  ((double)(tY[0] * _gyroCounts[0] + tY[1] * _gyroCounts[1] + tY[2] * _gyroCounts[2]) * _gyroScale) - _gyroB[1];
	_gyro[2] =
	  ((double)(tZ[0] * _gyroCounts[0] + tZ[1] * _gyroCounts[1] + tZ[2] * _gyroCounts[2]) * _gyroScale) - _gyroB[2];
	return 1;
}
 ...
 */
int16_t ICM20689::getAccelX_raw() {
	return _accCounts[0];
}
...
int ICM20689_FIFO::readFifo() {
	...
	for (size_t i = 0; i < _fifoSize / _fifoFrameSize; i++) {
		...
		if (_enFifoAccel) {
			...
			_axFifo[i] =
			  (((double)(tX[0] * _accCounts[0] + tX[1] * _accCounts[1] + tX[2] * _accCounts[2]) * _accelScale) - _accB[0])
			  * _accS[0];
			_ayFifo[i] =
			  (((double)(tY[0] * _accCounts[0] + tY[1] * _accCounts[1] + tY[2] * _accCounts[2]) * _accelScale) - _accB[1])
			  * _accS[1];
			_azFifo[i] =
			  (((double)(tZ[0] * _accCounts[0] + tZ[1] * _accCounts[1] + tZ[2] * _accCounts[2]) * _accelScale) - _accB[2])
			  * _accS[2];
		...
		if (_enFifoGyro) {
			// combine into 16 bit values
			_gyroCounts[0] = (((int16_t)_buffer[0 + _enFifoAccel * 6 + _enFifoTemp * 2]) << 8)
			               | _buffer[1 + _enFifoAccel * 6 + _enFifoTemp * 2];
			_gyroCounts[1] = (((int16_t)_buffer[2 + _enFifoAccel * 6 + _enFifoTemp * 2]) << 8)
			               | _buffer[3 + _enFifoAccel * 6 + _enFifoTemp * 2];
			_gyroCounts[2] = (((int16_t)_buffer[4 + _enFifoAccel * 6 + _enFifoTemp * 2]) << 8)
...
/* reads registers from ICM20689 given a starting register address, number of
